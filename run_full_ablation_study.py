...
class SingleEncoderClassifier(nn.Module):
    def __init__(self, in_features=768, num_classes=2):
        super().__init__()
...
def run_ablation_suite(data_path, text_col, class_col, dataset_name="Dataset"):
    print(f"\n=======================================================", flush=True)
    print(f"RUNNING SYSTEMATIC ABLATION STUDY ON: {dataset_name}", flush=True)
    print(f"=======================================================", flush=True)
    ...
    # Ablation Variants: (Name, Architecture, Augmentation, Weighting, LossType, Gamma)
    ablation_variants = [
        ("1. Single Encoder (BERT Only)", "single", False, False, "ce", 0.0),
        ("2. Dual Encoders (Concat)", "concat", False, False, "ce", 0.0),
        ("3. Dual Encoders + MHCA", "mhca", False, False, "ce", 0.0),
        ("4. Dual Encoders + MHCA + Augmentation", "mhca", True, False, "ce", 0.0),
        ("5. Dual Encoders + MHCA + Aug + Class Weighting", "mhca", True, True, "ce", 0.0),
        ("6. Full Model: MHCA + Aug + Class Weight + Focal Loss (gamma=1.0)", "mhca", True, True, "focal", 1.0),
        ("7. Full Model: MHCA + Aug + Class Weight + Focal Loss (gamma=2.0)", "mhca", True, True, "focal", 2.0),
        ("8. Full Model: MHCA + Aug + Class Weight + Focal-Attn Joint Opt", "mhca", True, True, "focal_attn", 2.0),
    ]
    
    results = []
    
    for name, arch, use_aug, use_weight, loss_type, gamma in ablation_variants:
        set_seed(42)
        ...
        for fold, (train_idx, val_idx) in enumerate(skf.split(texts, y)):
            ...
            if use_aug:
                X_tr, y_tr = apply_semantics_preserving_augmentation(X_tr_orig, y_tr_orig, min_samples=30)
            else:
            ...
            X_tr_b = torch.tensor(vec_b.fit_transform(X_tr).toarray(), dtype=torch.float32)
            X_tr_r = torch.tensor(vec_r.fit_transform(X_tr).toarray(), dtype=torch.float32)
            ...
            if use_weight:
                class_weights = compute_class_weight(class_weight='balanced', classes=classes_all, y=y_tr)
                weights_tensor = torch.tensor(class_weights, dtype=torch.float32)
            else:
                weights_tensor = None
                
            if arch == "single":
                model = SingleEncoderClassifier(in_features=768, num_classes=num_classes)
            elif arch == "concat":
                model = DualConcatClassifier(in_features=1536, num_classes=num_classes)
            elif arch == "mhca":
            ...
            for epoch in range(60):
                ...
                with torch.no_grad():
                    if arch == "single":
                        ...
                        val_logits = model(X_val_b, X_val_r)
                    elif arch == "mhca":
        ...
        print(f"  {name:<68} | Acc: {mean_acc*100:.2f}% (Peak: {peak_acc*100:.2f}%) | Macro F1: {mean_mf1:.4f} | Weighted F1: {mean_wf1:.4f}", flush=True)
        results.append((name, mean_acc, peak_acc, mean_mf1, mean_wf1))

run_ablation_suite('B:/Promise/nfr.csv', 'RequirementText', 'class', dataset_name="PROMISE NFR (625 samples)")
run_ablation_suite('B:/Promise.csv', 'Requirement', 'Type', dataset_name="PROMISE_EXP (969 samples)")
